...
This Python script demonstrates one way to generate an output file containing a subset of candidate fusion events in strip 3 of the MUSIC detector. These potential events are saved in a new file, which can then be used in an analysis pipeline that works with a smaller set of "interesting traces" rather than the entire event set. This example uses a simple single-value threshold on a dataset that has already been gain matched (normalized) and had pileup events removed. For each event, it first checks whether the preceding two strips (s1 and s2) are beam-like, then further slices the dataframe using the threshold condition on the long strip in strip 3. A final condition, usually appropriate for finding fusion events, checks the trace is below beam at the end of the detector. Those events that match those criteria are saved in a new feather file for further analysis.
